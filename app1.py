...
def detect(save_img=False):
    ...
    colors = [[random.randint(0, 255) for _ in range(3)] for _ in names]  # FIX: exactly 3 values for RGB
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):
            ...
            if len(det):
                ...
                person_class = 0
                persons = det[det[:, -1] == person_class]
                num_persons = persons.shape[0]
                total_count += num_persons

                s += f'{num_persons} person{"s" if num_persons != 1 else ""}, '
            ...
            cv2.putText(im0, f'Number of people: {num_persons}', (30, 50),
    ...
    if save_txt or save_img:
        saved_label_count = len(list((save_dir / 'labels').glob('*.txt'))) if save_txt else 0
        s = f"\n{saved_label_count} labels saved to {save_dir / 'labels'}" if save_txt else ''
...
if __name__ == '__main__':
    ...
    parser.add_argument('--classes', nargs='+', type=int, default=[0], help='filter by class: only detect persons by default')
